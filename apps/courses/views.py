...
from .models import CourseResouce, Video
from operation.models import CourseComments, UserCourse, CourseComments
...
class CourseInfoView(LoginRequiredMixin, View):
    def get(self, request, course_id):
        ...
        relate_courses = Course.objects.filter(id__in=course_ids).order_by("-click_nums")[:5] #获取学过该用户学过其他的所有课程
        all_resource = CourseResouce.objects.filter(course=course)
...
class CourseCommentView(LoginRequiredMixin, View):
    def get(self, request, course_id):
        course = Course.objects.get(id=int(course_id))
        comments = CourseComments.objects.filter(course_id=int(course_id))
        # 查询用户是否已经关联了该课程
        if comments:
            all_comments = comments.order_by("-add_time")
        else:
            all_comments = []
        ...
        all_user_courses = UserCourse.objects.filter(user_id__in=user_ids)
        course_ids = [user_course.course.id for user_course in all_user_courses]  # 取出所有课程ID
        relate_courses = Course.objects.filter(id__in=course_ids).order_by("-click_nums")[:3]  # 获取学过该用户学过其他的所有课程
